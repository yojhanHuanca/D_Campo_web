...
@php
    $favoritosCount = isset($totalFavoritos) ? $totalFavoritos : (Auth::check() ? Auth::user()->favoritos()->count() : 0);
    $pedidosCount = $totalPedidos ?? 0;
@endphp
    ...
    <div class="card-body text-center p-3">
        ...
        <div class="d-flex justify-content-around align-items-center mb-2">
            <div class="text-center">
                <span class="badge bg-danger rounded-pill px-3 py-2 d-inline-block" style="min-width: 38px;">{{ $pedidosCount }}</span>
                <small class="text-muted d-block" style="font-size: 0.7rem;">Pedidos</small>
            </div>
            <div class="text-center">
                <span class="badge bg-danger rounded-pill px-3 py-2 d-inline-block" style="min-width: 38px;">{{ $favoritosCount }}</span>
                <small class="text-muted d-block" style="font-size: 0.7rem;">Favoritos</small>
            </div>
        </div>
    ...
    <ul class="list-group list-group-flush">
        ...
        <a href="{{ route('perfil.pedidos') }}"
           class="list-group-item list-group-item-action d-flex align-items-center justify-content-between py-2">
            ...
            <span class="badge bg-danger rounded-pill">{{ $pedidosCount }}</span>
        </a>
        ...
        <a href="{{ route('perfil.favoritos') }}" 
           class="list-group-item list-group-item-action d-flex align-items-center justify-content-between py-2">
            ...
            <span class="badge bg-success rounded-pill">{{ $favoritosCount }}</span>
        </a>
